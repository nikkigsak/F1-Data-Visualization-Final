...
standings_over_time <- left_join(driver_race_standings, xover_total, by='driverId')
standings_over_time <- rename(standings_over_time, year = year.x)

races2021 <- standings_over_time %>% filter(year==2021)
races2019 <- standings_over_time %>% filter(year==2019)

ggplot(races2021, aes(x=date,y=position,group=full_name, color=full_name, lientype = factor(rn))) + geom_line() + 
  scale_linetype_manual(values=races2021$line_type) + coord_flip() + ggtitle("2021") + theme_gray()
ggplot(races2019, aes(x=date,y=position,group=full_name, color=full_name)) + geom_line() + coord_flip() + ggtitle("2019") + theme_gray()
...
ggplot(races2021, aes(x=date,y=points,group=full_name,color=full_name)) + geom_point() + geom_smooth(se=FALSE) + coord_flip() + ggtitle("2021") + theme_gray()
ggplot(races2019, aes(x=date,y=points,group=full_name,color=full_name)) + geom_point() + geom_smooth(se=FALSE) + coord_flip() + ggtitle("2019") + theme_gray()
