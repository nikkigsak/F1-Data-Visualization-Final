...
lap_times_by_driver_2021 <- 
  ...
  left_join(xover, by = "driverId","year") %>%
  group_by(year.x, raceId, driverId, full_name, constructor_name) %>%
  summarise(max(milliseconds)) %>%
  rename(fastestLap = "max(milliseconds)") %>%
  filter(year.x == 2019) %>%
  mutate(fastestLap = fastestLap/1000) %>%
  group_by(raceId) %>%
  mutate(rank = rank(fastestLap)) %>%
  filter(rank == 1) %>%
...
lap_times_by_driver_2019 %>%
  plot_ly(type = "bar",
          ...
          y = ~full_name,
          marker = list(color =c("#9B0000", # Antonio Giovinazzi (Alfa Romeo)
                                 "#FF8700", # Carlos Sainz (2019 at McL; 2021 at RB)
                                 "#FFF500", # Ricciardo Renault 2019, 2021 at McL
                                 "#469BFF", # Kvyat at Toro Roso/Alpha Tauri
                                 "#FFFFFF", # George Russell (williams 2019, mb 2021)
                                 "#FF8700", # Norris, McLaren
                                 "#00D2BE", # Lewis Hamilton; Mercedes
                                 "#1E41FF", # Max Verstappen; Red Bull
                                 "#FFF500", # Nico Hulkenberg; Renault 2019
                                 "#469BFF", # Gasly Toro Rosso 2019/Alpha Tauri 2021
                                 "#DC0000", # Vettel Ferrari 2019; Aston Martin 2021
                                 "#F596C8", # Perez, Racing Point 2019, RB 2021
                                 "#00D2BE")),
 #         text = ~constructor_name,
          hovertemplate = paste('<b>Team: %{text}</b>', 
                                 '<br><b>Number of Fastest Laps</b>: %{x}<br>')) %>%
...
lap_times_by_driver_2021 %>%
  plot_ly(type = "bar",
          x = ~count_of_fastest_lap, 
          y = ~full_name,
          marker = list(color =c("#B12039", # Antonio Giovinazzi (Alfa Romeo)
                            "#ED1C24", # LeClerc; Ferrari
                            "#2293D1", # Ocon, Alpine 2021
                            "#6CD3BF", # George Russell mercedes 2021
                            "#B12039", # Kimi at Alfa Romeo
                            "#2D826D", # Stroll; Racing Point 2019; AM 2021
                            "#F58020", # Norris, McLaren
                            "#6CD3BF", # Lewis Hamilton; Mercedes
                            "#1E5BC6", # Max Verstappen; Red Bull
                            "#B6BABD", # Mick Schumacher; Haas
                            "#1E5BC6", # Perez, Racing Point 2019, RB 2021
                            "#6CD3BF", # Bottas Merc 
                            "#4E7C9B" # Yuki!! Tsonoda! Alpha Tauri
                            )),
          text = ~constructor_name,
          hovertemplate = paste('<b>Team: %{text}</b>', 
                                 '<br><b>Number of Fastest Laps</b>: %{x}<br>')) %>%
...
I'm creating a walkover between drivers and teams by season so I can use this. 
```{r}
library(lubridate)
...
incidents_by_circuit %>%
  ...
  do(p = plot_ly(type = "bar",
          ...
          marker = list(color =c("#B12039", # Antonio Giovinazzi (Alfa Romeo)
                        ...
                        hoverinfo = "text", 
                        hovertext = ~n) %>%
  layout(title = 'Incidents by ', xaxis = list(title = 'Count of Fastest Laps'), 
         yaxis = list(title = 'Driver'))) %>%
